...
def add_parser(subparsers):
    parser = subparsers.add_parser("update-docref", help="Update the doc references")
...
def fetch_pages(lib):
    ...
    for tag in soup.findAll("sub"):
        page = tag["link"]
        if page.startswith(("index-", "api-index-", "annotation-glossary", "ix")):
            continue
        if "#" in page:
            continue
        pages.add(page)
    ...
    with ThreadPool(30) as pool:
        for lib in LIBRARIES:
            ...
            with progress(len(pages)) as update:
                for i, (names, page) in enumerate(
                    pool.imap_unordered(fetch_page, [(lib, p, keywords) for p in pages])
                ):
                    update(i + 1)
            ...
            with open(get_docref_path(namespace, version), "wb") as h:
                h.write(json.dumps(mapping, sort_keys=True, indent=4).encode("utf-8"))
